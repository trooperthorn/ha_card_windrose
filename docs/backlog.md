...
| # | Title | Effort | Notes |
| --- | --- | --- | --- |
| [201](https://github.com/aukedejong/lovelace-windrose-card/issues/201) | Set card height explicitly, plus vertical content alignment | S | `card_width` already exists as a precedent; a `card_height` option and a top/middle/bottom alignment knob follow the same pattern. |
| [207](https://github.com/aukedejong/lovelace-windrose-card/issues/207) | Hide or relabel a corner value when its source entity is `unknown` | S | Config-only: an `unknown_value` override string (default keeps current behavior), or a `hide_when_unknown` flag on `corner_info` entries. |
| [211](https://github.com/aukedejong/lovelace-windrose-card/issues/211) | Reposition or reformat corner values that have no label | S/M | Requester is non-technical but the ask is scoped: layout options for corner_info when `show_label: false`. Needs a look at the current renderer to see how much is hardcoded. |
| [165](https://github.com/aukedejong/lovelace-windrose-card/issues/165) | Color `corner_info` values by value range (reuse `speed_ranges` pattern) | S/M | Requester already sketched the config shape, modeled directly on the existing `speed_ranges` structure. Mostly a matter of threading a similar lookup into corner rendering. |
| [127](https://github.com/aukedejong/lovelace-windrose-card/issues/127) | Drive `hours_to_show` from a template/helper (e.g. `input_number`) instead of a static number | S/M | Depends on whether the card already supports templated config values anywhere (Lovelace-level templating via `config-template-card` composition is the usual workaround). Worth checking if this is already possible via existing card composition before adding native support. |

## Rendering / data-pipeline features (medium)

| # | Title | Effort | Notes |
| --- | --- | --- | --- |
| [226](https://github.com/aukedejong/lovelace-windrose-card/issues/226) | `data_period.from_date` / `to_date` (fixed range) not accepted, only relative `hours_to_show` | M | Reporter got "No data period config option available" using an explicit date range. Real gap: today's `data_period` schema is relative-only. Needs a schema change plus statistics-query changes to accept an absolute range. |
| [225](https://github.com/aukedejong/lovelace-windrose-card/issues/225) | Text-block variables (`{max-speed}` etc.) only resolve for the first `windspeed_entities` entry | M | Bug-adjacent feature gap: multi-entity configs lose access to per-entity template variables. Also wants min/max paired with the timestamp and direction they occurred at, which is a data-shape change, not just a template fix. |
...
| [125](https://github.com/aukedejong/lovelace-windrose-card/issues/125) | Additional info block above/below the rose (mirrors `corner_info` structure) | M | Requester proposes reusing the existing `corner_info` object shape under a new `top_info`/`bottom_info` key, which keeps the config surface consistent with what already exists. |
