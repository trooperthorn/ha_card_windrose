...
| average_period_back          |            [period code](#Period-code-explained)             |     (uses data_period)       |    -     | Only with dynamic_speed_ranges: use a separate, independent time window to compute the average that selects the speed range step/max, instead of averaging over the rose's own display period. For example `-7d` picks the dynamic range from a week's average while the rose itself still shows the last day. |
...
| min-speed<br/>max-speed<br/>average-speed                                                     | Wind speed statistics for the active windspeed entity (the one currently used for the windrose).                                                    | 
| min-speed-x<br/>max-speed-x<br/>average-speed-x                                               | Same statistics for a specific `windspeed_entities` entry. The x should be the index of the windspeed sensor, 0 for the first. Not only the active entity: with two windspeed_entities configured, `${max-speed-1}` is available even while entity 0 is active.|
| min-speed-time<br/>min-speed-direction<br/>max-speed-time<br/>max-speed-direction             | When and from which wind direction the minimum/maximum speed occurred, for the active windspeed entity. Also available per entity as `min-speed-time-x` etc. |
...
| median-speed<br/>median-speed-x                                                               | Median windspeed (50th percentile), for the active entity or a specific `windspeed_entities` index.                                                 |
| q1-speed<br/>q1-speed-x                                                                       | Interquartile range (25th percentile)                                                                                                               |
| q3-speed<br/>q3-speed-x                                                                       | Interquartile range (75th percentile)                                                                                                               |
| iqr-range<br/>iqr-range-x                                                                     | Interquartile range (25th to 75th percentile) - excludes outliers                                                                                   |
| p90-speed<br/>p90-speed-x                                                                     | 90th percentile for gusts (excludes the highest 10% outliers)                                                                                       |
| wind-description<br/>wind-description-x                                                       | Weather-style description using IQR                                                                                                                 |
